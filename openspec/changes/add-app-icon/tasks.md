...
- [ ] 1.1 Author `public/icon.svg`: an ink rounded-square field (`#23213a`), a thick gold
...
- [ ] 1.2 Confirm the mark survives tab size. **Check:** rendered at 16×16 the ring, its
...
- [ ] 1.3 Produce `public/icon-180.png`, `public/icon-192.png` and `public/icon-512.png`
...
- [ ] 2.1 Add to `index.html`'s `<head>`: `<link rel="icon" type="image/svg+xml"
...
- [ ] 2.2 Write `public/manifest.webmanifest` with `name`, `short_name`, the three PNGs
...
- [ ] 3.1 Add `tests/app-icon.test.ts`: read `index.html`, extract the `icon`,
...
- [ ] 3.2 In the same test, parse `public/manifest.webmanifest` and assert every icon it
...
- [ ] 3.3 Prove the test actually catches both failures. **Check:** temporarily rename one
...
- [ ] 3.4 Assert nothing is fetched from a third party: every icon reference in
...
- [ ] 4.1 Update `README.md`: where the mark lives, that `icon.svg` is the source, and
...
- [ ] 4.2 Update `docs/PLAN.md` §8 — the project tree gains `public/` — and add a `D-22`
...
- [ ] 4.3 Run the full gate. **Check:** `npm run typecheck`, `npm test` and `npm run build`
...
- [ ] 5.5 Play a lesson with the network turned off after load. **Check:** every exercise
  behaves exactly as before this change, whether or not any icon was ever fetched (spec
  "Playing with the network gone").
- [ ] 5.6 Report which checks were automated and which were performed by hand, in the
