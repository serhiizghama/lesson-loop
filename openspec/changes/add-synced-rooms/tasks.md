...
- [ ] 1.1 Add `src/shared/protocol.ts`: the client→room messages (`hello`, `action`,
...
- [ ] 1.2 Add `src/shared/room.ts` with `RoomCore`: it holds `{ lessonId, state, locked,
...
- [ ] 1.3 Unit-test `RoomCore` against the `synced-rooms` spec's own terms: an action from
...
- [ ] 1.4 Write the transport-free convergence test (design D19): two simulated clients and
...
- [ ] 1.5 Assert the snapshot stays small enough for D10's full-state broadcast to be the
...
- [ ] 2.1 Extend `src/shared/blocks/contract.ts` with `answerKey(lesson, block, state)`
...
- [ ] 2.2 Implement `answerKey` for `cards`, `match`, `sentence`, `sort` and `listen`: the
...
- [ ] 2.3 Return `null` from `tpr` and `finish`, which score nothing. **Check:** the test
  asserts `null`, and the `teacher-view` scenario "An exercise with no answer" has a test to
  point at.

## 3. The Worker and the Durable Object

- [ ] 3.1 Add `wrangler` and `@cloudflare/workers-types` as dev dependencies and
...
- [ ] 3.2 Write `worker/index.ts`: `POST /api/rooms` creates a room (4-character code from an
...
- [ ] 3.3 Write `worker/room.ts`: a thin Durable Object over `RoomCore` that assigns the role
...
- [ ] 3.4 Persist `RoomState` to `ctx.storage` on every mutation and restore it in the
...
- [ ] 3.5 Set an `alarm()` that discards the room after three hours of inactivity, rescheduled
...
- [ ] 3.6 Include `worker/` in the TypeScript project. **Check:** `npx tsc --noEmit` covers
  `worker/` and CI fails on a type error introduced there.

## 4. The network client and solo fallback

- [ ] 4.1 Add `src/net/socket.ts`: opens the connection, sends typed protocol messages, and
...
- [ ] 4.2 Add `src/ui/useRoom.ts` returning the same `{ state, dispatch, progress }` shape as
...
- [ ] 4.3 Surface connection status through that hook and show "working without sync" after
...
- [ ] 4.4 Confirm the solo path is untouched: opening a lesson from the home screen still
...
- [ ] 5.1 Replace the `useState` lesson picker in `src/ui/App.tsx` with a hand-rolled router
...
- [ ] 5.2 Add the "Invite student" action to a lesson in progress: it creates the room from
...
- [ ] 5.3 Make the invitation idempotent — asking again offers the same link rather than
  creating a second room. **Check:** the teacher's link is unchanged after asking twice, and
  `POST /api/rooms` was called once.
- [ ] 5.4 Handle an unknown, expired or full room on entry by explaining it and offering the
...
- [ ] 6.1 Build the teacher panel beside the exercise: previous/next, reset this exercise,
...
- [ ] 6.2 Render the answer key for the exercise on screen from `answerKey`, showing nothing
...
- [ ] 6.3 Add the lock toggle, enforced in `RoomCore` and reflected in the student's UI
  ...
  student's next tap.
- [ ] 6.4 Show whether a student is connected and whether the teacher's own connection is
...
- [ ] 6.5 Make the panel dismissible and keep the exercise fully visible at 380 px wide
...
- [ ] 7.1 Serve `/r/<code>` with the exercise, its instruction and progress, and nothing that
...
- [ ] 7.2 Land a joiner on the exercise currently in play with all progress so far (spec "The
...
- [ ] 8.1 Update `README.md` with what a room is, the two links, how to run the Worker and the
  ...
  source file.
- [ ] 8.2 Update `docs/PLAN.md`'s decision table with the decisions taken in this change that
...
- [ ] 8.3 Extend `.github/workflows/ci.yml` if the added dev dependencies or the `worker/`
  directory need it. **Check:** the workflow is green on the branch, running typecheck, test
  and build.
- [ ] 8.4 Run the full gate. **Check:** `npx tsc --noEmit`, `npm test` and `npm run build` all
  pass, and the result is reported honestly, naming anything skipped.

## 9. Acceptance

- [ ] 9.1 Play a whole lesson in two browser windows under `wrangler dev`, one on each link,
  ...
  both.
- [ ] 9.2 Switch lesson mid-session from the teacher's window (D-11). **Check:** the student
  follows to the new lesson at its first exercise on the same link they were given.
- [ ] 9.3 Break the connection deliberately mid-exercise and restore it. **Check:** both
  screens stay playable and say they are unsynced, and on restoration they converge on the
  same state without a reload.
- [ ] 9.4 Report which checks were automated and which were performed by hand (design D19).
