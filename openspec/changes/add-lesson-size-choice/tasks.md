...
- [ ] 1.1 Add `Part = { id: string; title: string; emoji: string; items: string[] }` and
...
- [ ] 1.2 Add `{ select: 'new' }` to `ItemRef` and `only?: string[]` to `BlockBase`.
...
- [ ] 1.3 Extend the zod schemas in `src/shared/validate.ts` for `parts`, `select:'new'`
...
- [ ] 1.4 Confirm every current lesson file is still valid with no edits.
  *Check:* `npm test -- lessons` passes untouched.

## 2. Narrowing

- [ ] 2.1 Write `narrow(lesson, choice)` in a new `src/shared/narrow.ts`, where `choice`
...
- [ ] 2.2 Make `narrow` erase every trace of the topic: no `parts`, no `only`, no
...
- [ ] 2.3 Add `choicesOf(lesson)` returning the sizes a topic offers — each with its id,
...
- [ ] 2.4 Keep `src/shared` pure.
  *Check:* `npm test -- purity` passes.

## 3. Every size is validated

- [ ] 3.1 Add the structural checks on `parts` before any narrowing: an item id no item
...
- [ ] 3.2 Run the existing cross-checks over every derived size, prefixing each error with
...
- [ ] 3.3 Check `match.count` and `listen.choices` against each size, not only the file.
  *Check:* a topic whose part-one lesson has fewer items than a block's `count` fails,
  naming the size.

## 4. Addresses

- [ ] 4.1 Extend `parseRoute` with `/l/<topic>/<choice>` and keep `/l/<topic>`; add
  `lessonPath(topicId, choiceId?)` (design D5).
  *Check:* `router.test.ts` covers both forms, an unknown choice, and that `/r/` and `/t/`
  are unchanged.
- [ ] 4.2 In `App`, resolve a route to a topic and a choice: narrow before rendering, and
...
- [ ] 4.3 Key `SoloLesson` on topic-plus-choice so switching size starts fresh (design D7).
  *Check:* a test completes an exercise on one size, switches to another, and finds no
  progress and no stars carried over.
- [ ] 4.4 Confirm a room still opens on whatever lesson is in play, with no worker change.
...
- [ ] 5.1 Make a topic card with parts expand in place to its sizes, each saying what it
...
- [ ] 5.2 Label the whole-topic choice with the topic's own word count, not a fixed number.
  *Check:* a test asserts Animals offers "all 10" and Shapes "all 8".
- [ ] 5.3 Keep the card readable at 380px with the sizes expanded.
...
- [ ] 6.1 Copy `animals-1`, `animals-2`, `shapes-1`, `shapes-2` into
  `src/shared/__fixtures__/` as the migration's reference.
  *Check:* the fixtures load and validate as ordinary lessons.
- [ ] 6.2 Author `lessons/animals.json`: ten items, parts `known` and `wild`, teaching
...
- [ ] 6.3 Author `lessons/shapes.json` the same way: eight items, parts `basic` and `more`,
...
- [ ] 6.4 Delete `animals-1.json`, `animals-2.json`, `shapes-1.json`, `shapes-2.json`, and
...
- [ ] 8.2 Remove "re-cutting the remaining four lessons to five words each" from what
  remains of v0.2, and say what replaced it.
  *Check:* §9 no longer lists it.
- [ ] 8.3 Update `README.md` where it says a topic longer than five words becomes another
  file.
