...
carry, when a selected item has no place on the drawing, or when a place falls outside the
drawing.
...
and free text unattached to a vocabulary item SHALL NOT enter the lesson format.
...
lessons in full, including the labelled body diagram, with no lesson-specific code path
...
- **THEN** every exercise they contained behaves as it did in the original material, and no
  code branches on the identity of either lesson
