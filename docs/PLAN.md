...
- **Home** — a grid of topic tiles (emoji). A topic that declares parts expands in place
  to the sizes it can be taught in — each part, and the whole of it — and tapping one opens
  that lesson; a topic with no parts opens directly (D-43, D-47). Either way the lesson runs
  locally, with no network at all. A room is created only when the teacher asks to invite a
  student, from inside the lesson, carrying whatever progress has already been made (D-13).
...
├─ lessons/          # animals.json, shapes.json, body-parts.json, ... — one file a topic
...
not about software: **a lesson is five words** (D-36). Animals became `animals-1` and
`animals-2` — later merged back into one topic with two declared parts, when the *number*
turned out to be hers to choose rather than ours to file (D-43). The second was a new topic, Shapes, most of whose exercises the engine could
...
`add-lesson-size-choice` **gives the choice of how long a sitting is back to the teacher**,
and with it retires the re-cutting that was the rest of v0.2. A topic is one file that
declares its parts and she picks the size when she opens it (D-43); the four topics that
were waiting to be cut gained parts instead, at no cost in pictures or recordings. The
`-1`/`-2` files are gone: `animals-1` + `animals-2` are `animals.json`, `shapes-1` +
`shapes-2` are `shapes.json`, and the migration is pinned by a test that narrows the merged
topic back to each file it replaced (D-44, D-46).

v0.2 closes here. Tablet polish, the last item on its list, is deferred rather
than delivered: what a tablet browser does today is not broken, and what would be
polished is still a guess about a device nobody has confirmed is in use (D-49).

**Between v0.2 and v0.3 — `add-teacher-guide`.** Not on either list, and not a feature:
the app now explains itself to the person teaching with it. `/guide` is eleven sections,
about 1200 words and eight screenshots of the real app, written for the teacher and never
mentioning how it is built. It is offered in her panel inside a room and nowhere else, and
opens in a second tab so reading it never takes a live lesson off her screen (D-50). The
part that matters after today is that the exercise table is checked against the block
registry: a thirteenth block type cannot reach `main` without a line describing it.
...
| D-36 | ~~**A lesson is five words, not ten.**~~ **Superseded by D-43…D-47** (`add-lesson-size-choice`, 2026-09-10). The teaching finding stands and is where every part still comes from — a child who has met ten animals in forty minutes has been shown ten, not taught ten. What is withdrawn is the second half of the decision: that the rule belongs in the filing, so a topic longer than five words becomes several files and the format has no notion of parts. That made the app decide the size of every sitting on the teacher's behalf, and the evidence never supported the number it decided (her Animals is 5+5; her Shapes is 4+4). A topic is one file declaring its parts, and she chooses | 2026-09-09 |
...
| D-43 | **A topic is one file that declares its parts, and the teacher chooses the size when she opens it.** This withdraws the filing half of D-36. The parts are hers — the halves she drew herself — moved out of the filenames and into the file: each names its own items and carries its own title. What she is offered when she taps a topic is each part and the whole of it; what starts is the lesson that choice builds. The app keeps the *shape* of her finding (a topic has halves, a later half revises the earlier one, a sitting introduces a handful) and stops deciding which of those sittings today is. The cost of the old arrangement was not only pedagogical: four topics were still waiting to be cut by hand, and each cut was a new file to author and keep in step | 2026-09-10 |
| D-44 | **A size is a lesson that was built, never a setting that travels.** `narrow(topic, choice)` is a pure function in `src/shared` that runs before anything is played and returns an ordinary `Lesson`: no parts, no `only`, no `select: 'new'`. So the player, the reducer, the room, the Durable Object and the wire protocol were untouched by this change, and no test of them changed. Carrying the choice in `LessonState` was rejected — it would put a lesson-authoring concept into the state broadcast on every tap, for a choice that never changes once a lesson has started. Narrowing in the Worker was rejected too: the solo path needs it anyway, so it would have to exist in two places | 2026-09-10 |
| D-45 | **A part is cumulative in vocabulary and exclusive in exercises.** Choosing part *k* carries the words of parts 1..*k* and teaches part *k* alone; a block belongs to the sittings its `only` names, and to every sitting when it names none. The asymmetry is the difference between what a sitting *has* and what a sitting *is*: part two of Animals needs ten words so that "Where do they live?" has a second bucket (D-37), and needs part one's model phrases left behind, because they quote part one's words. Making blocks cumulative too was rejected — a child learning stars would be walked through "It is a circle." first. So was dropping a block automatically when its selection came out too small: implicit where `only` is explicit, unable to express the phrases case at all, and a block that vanishes silently is discovered mid-lesson | 2026-09-10 |
| D-46 | **A topic is validated at every size it offers, not only as it is written.** A file can be sound as a whole and unsound as a sitting — a sorting exercise whose buckets all fill from one part is no question at all once that part is played alone, and a match asking for six pairs cannot be finished by a sitting of five. `validateLesson` checks the parts structurally, then narrows to each size and runs every existing cross-check over the result, naming the size that failed. This is the check that would have caught the defect in the teacher's own part two, and it turns the exercise the change is proudest of into a build error rather than a dull minute in front of a child | 2026-09-10 |
| D-47 | **The choice rides in the address and expands on the topic's own card.** `/l/<topic>/<choice>`, so a size opens cold, survives a reload and keys the player — changing size starts the lesson again with no explicit reset. `/l/<topic>` is the topic with its sizes showing, which is the honest answer for a topic that is no longer one lesson, and it is the home screen rather than a screen of its own: no step between the teacher and a lesson she has already chosen, and nothing at all for a topic with no parts. Each size says what it teaches ("5 new words · Animals 2 · Wild Animals"), because a bare 5 / 10 asks her to remember which half is which; the whole-topic choice counts the topic, since Shapes has eight. **Breaking:** `/l/animals-1`, `/l/animals-2`, `/l/shapes-1` and `/l/shapes-2` no longer resolve — they are in nobody's hands, a student holds `/r/<code>`, and the app has one user | 2026-09-10 |
| D-48 | **A built lesson is named after its size — `animals/wild` — because a room finds its lesson by name.** The lesson never crosses the socket: both participants resolve it from their own copy of `lessons/` by the id in the state, and the reducer refuses an action whose state names a different lesson than the one on screen. So every size a teacher can open has to be findable by name, and `narrow` gives what it builds the id of its address. This qualifies D-44 rather than contradicting it: no message shape changes and nothing downstream learns what a part is, but the *identity* of the lesson does travel — exactly as `animals-2` travelled when it was a file. Sending the lesson on join was rejected: it is the cleaner architecture and it is a change to the room protocol, which this change set out not to touch, and the solo path would need `narrow` regardless. Found by opening a room on a part in a real browser, where a joiner got a blank screen; no unit test could have seen it, because every one of them held the lesson it was testing | 2026-09-10 |
| D-49 | **Tablet polish is deferred until someone says they teach from a tablet, and v0.2 closes without it.** It was put on the v0.2 list before the first real lesson, on §7's assumption that the student is "most likely on a tablet" — which is exactly what §11's second question asks the teacher and she has not yet answered. Nothing about a tablet is broken today: the lesson plays, the room syncs, taps land, sound works. What reading the code found is four defects of comfort rather than of function — `stageScale` measures width only, so a tablet in landscape scrolls the exercise instead of fitting it; neither `user-select` nor `-webkit-touch-callout` is set, so a long press on a card raises the iOS selection menu; `InkLayer` filters no `pointerType` and guards no second pointer, so a resting palm cuts a stylus stroke in two; and `height: 100%` jumps under Safari's collapsing toolbars. Each is cheapest to fix against a device actually in use, and each is recorded here so the reading is not repeated. Polishing now would make the error D-36 made and D-43 undid: deciding on her behalf a thing only she can answer | 2026-09-10 |
| D-50 | **The app carries its own guide for the teacher, and the entry to it is in her panel inside a room — nowhere else.** Everything she can do had been explained in chat messages; the only written account of the app is this file and `README.md`, both addressed to whoever builds it. The guide is a page of the app rather than a document beside it, so its content is reachable from the test suite — which is the whole point: the exercise table is checked against the block registry, the pictures against the files, and the prose against a list of construction words. Documentation nobody is forced to update stops being true within two changes. The entry was first put in the home screen's brand band and moved on the client's instruction: the panel is the one surface built for the teacher alone, so a child cannot reach the guide by construction rather than by styling. It opens in a second tab, because a lesson in progress is not navigated away from to read documentation; reading it before a first lesson is served by the address itself | 2026-09-10 |
